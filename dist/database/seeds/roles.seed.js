...
async function seedRolesAndPermissions(connection) {
    const permissionRepository = connection.getRepository(permission_entity_1.Permission);
    ...
    await Promise.all(roles.map(async (roleData) => {
        ...
        if (!existingRole) {
            const role = roleRepository.create({
                ...
                permissions: savedPermissions.filter(p => roleData.permissions.includes(p.name)),
            });
