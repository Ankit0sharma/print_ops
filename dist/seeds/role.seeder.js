    ...
    async run(dataSource) {
        ...
        const roles = [
            ...
            },
            {
                ...
                permissions: savedPermissions.filter(p => ['view_jobs', 'create_jobs', 'edit_jobs', 'view_customers', 'manage_customers'].includes(p.name)),
            },
            {
                name: 'Designer',
                description: 'Handle design tasks',
                permissions: savedPermissions.filter(p => ['view_jobs', 'edit_jobs'].includes(p.name)),
            },
            {
                name: 'Production',
                description: 'Handle production tasks',
                permissions: savedPermissions.filter(p => ['view_jobs', 'view_materials', 'manage_materials'].includes(p.name)),
            },
            {
                name: 'Sales',
                description: 'Handle sales and customers',
                permissions: savedPermissions.filter(p => ['view_jobs', 'create_jobs', 'view_customers', 'manage_customers'].includes(p.name)),
            },
            {
                name: 'Accounting',
                description: 'Handle financial tasks',
                permissions: savedPermissions.filter(p => ['view_jobs', 'view_customers'].includes(p.name)),
            },
        ];
